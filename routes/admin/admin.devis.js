...
const {
  PAGINATION_LIMIT_ADMIN,
  TYPE_DOCUMENT_FACTURE,
  TYPE_DOCUMENT_DEVIS,
    ...
    let commandes = await Commande.findAll({
      ...
      include: [
        ...
        },
        {
          ...
          where: {
            ess_repetition: {
              [Op.between]: [
                check_value_date_start(ess_repetition),
                check_value_date_last(ess_repetition_2),
              ],
            },
          },
        },
            ...
            [Op.and]: [
              {
                doc_date: {
                  [Op.between]: [
                    check_value_date_start(doc_date),
                    check_value_date_last(doc_date_2),
                  ],
                },
              },
        ...
        },
        {
          ...
          where: {
            exp_depart: {
              [Op.between]: [
                check_value_date_start(exp_depart),
                check_value_date_last(exp_depart_2),
              ],
            },
          },
          required: false,
      ...
      where: {
        [Op.and]: {
          ...
          com_debut_spectacle: {
            [Op.between]: [
              check_value_date_start(com_debut_spectacle),
              check_value_date_last(""),
            ],
          },
          com_fin_spectacle: {
            [Op.between]: [
              check_value_date_start(com_fin_spectacle),
              check_value_date_last(""),
            ],
          },
        },
...
router.get("/view/:commandeId", async (req, res) => {
  ...
  try {
    ...
    let last_document_essayage = await Document.findAll({
      ...
      where: {
        [Op.and]: [
          { tdo_id: TYPE_DOCUMENT_BON_ESSAYAGE },
          { com_id: commande.com_id },
        ],
      },
      order: [["doc_date", "DESC"]],
    ...
    let last_document_livraison = await Document.findAll({
      ...
      where: {
        [Op.and]: [
          { tdo_id: TYPE_DOCUMENT_BON_LIVRAISON },
          { com_id: commande.com_id },
        ],
      },
      order: [["doc_date", "DESC"]],
...
router.post("/commande/delete/essayage", async (req, res) => {
  ...
  try {
    ...
    req.session.flash = {
      message: "Date d'essayage supprimé",
      type: "success",
    };
    res.redirect(`/admin/devis/view/${com_id}`);
  } catch (error) {
...
router.post("/commande-retour", async (req, res) => {
  ...
  try {
    ...
    if (parseInt(ret_nbre) <= parseInt(detail_exp.dex_nbre)) {
      ...
      req.session.flash = {
        message: "Valeur saisi est supérieur au nombre des produits expediés.",
        type: "danger",
