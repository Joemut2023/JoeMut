...
const { check_value_date_start, check_value_date_last } = require('../../helpers/check_search_input_date');
...
router.post('/search',async (req,res)=>{
    ...
    try {
        let detail_expeditions = await Detail_expedition.findAll({
            include : [
                ...
                },
                {
                    ...
                    where:{
                        exp_depart:{
                            [Op.between]:[check_value_date_start(exp_date),check_value_date_last(exp_date_2)]
                        }
                    }
                }
            ]
        ...
        res.render('produits.location/index',{
            detail_expeditions,
            moment,
            pro_ref,com_num,cli_nom,cli_prenom,exp_date,exp_date_2,
            check_front_input
        });
    } catch (error) {
        res.render('produits.location/index',{
